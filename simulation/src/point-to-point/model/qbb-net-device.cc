...
namespace ns3 {
	...
	Ptr<Packet> RdmaEgressQueue::DequeueQindex(int qIndex){
		...
		if (qIndex >= 0){ // qp
			...
			//这个回调函数调用rdmahw里的getNxtPacket函数
			m_rrlast = qIndex;
			m_qlast = qIndex;
		...
		QbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::QbbNetDevice")
			...
			.AddTraceSource ("QbbPfcSent", "send a PFC packet. 0: resume, 1: pause",
					MakeTraceSourceAccessor (&QbbNetDevice::m_tracePfcSent))
			;
	...
	void
		QbbNetDevice::TransmitComplete(void)	//一个packet传输完成后，会紧接着又调出一个packet进行传输
	{
		NS_LOG_FUNCTION(this);
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		Ptr<Packet> p;
		//首先是endhost需要发包，在rdmaEgressQueue中做
		if (m_node->GetNodeType() == 0){
			...
			if (qIndex != -1024){
				...
				//if(m_node->GetId()==17) printf("PAUSE prohibits send at node \n");
			...
			//if we choose to use perflow queue, still round-robin
	...
	//this function seems to be used to send back to the upstream link
	...
	void QbbNetDevice::SendPfc(uint32_t qIndex, uint32_t type){
		...
		p->PeekHeader(ch);
		SwitchSend(0, p, ch);
		if(type==0) m_tracePfcSent(1);
		else m_tracePfcSent(0);
	}
