...
namespace ns3 {
	...
	uint32_t SwitchMmu::GetPfcThreshold(uint32_t port){
		//uint32_t pfcThreshold=(buffer_size - total_hdrm - total_rsrv - shared_used_bytes) >> pfc_a_shift[port];
		//std::cout<<pfcThreshold<<"\n";
		
		return (buffer_size - total_hdrm - total_rsrv - shared_used_bytes) >> pfc_a_shift[port];
		
		//return buffer_size>>pfc_a_shift[port]-headroom[port]/3;
	}
