...
TypeId RdmaDriver::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaDriver")
		...
		// .AddTraceSource ("SendPkt", "A qp send pkt.",
        // 		MakeTraceSourceAccessor (&RdmaDriver::m_traceSendPkt))
		.AddTraceSource("changeRate", "change Rate record.",
				MakeTraceSourceAccessor (&RdmaDriver::m_traceChangeRate))
		;
	return tid;
}
...
void RdmaDriver::Init(void){
	...
	m_rdma->Setup(MakeCallback(&RdmaDriver::QpComplete, this), 
				//MakeCallback(&RdmaDriver::SendPkt, this),
				MakeCallback(&RdmaDriver::changeRate, this));
}
...
//
// void RdmaDriver::SendPkt(Ptr<RdmaQueuePair> qp, uint32_t seq, uint32_t expectSeq){
// 	//if(qp->sip.Get()== 0x0b000201)
// 		m_traceSendPkt(qp, seq, expectSeq); 
// }

void RdmaDriver::changeRate(uint32_t seq, double new_rate, double old_rate, double cur_rate, double c, Ipv4Address sip, Ipv4Address dip){
	m_traceChangeRate(seq, new_rate, old_rate, cur_rate, c, sip, dip);
}
