...
void IntHeader::PushHop(uint64_t time, uint64_t bytes, uint32_t qlen, uint64_t rate, uint8_t histogram[8]){
	// only do this in INT mode
	if (mode == NORMAL){
		uint32_t idx = nhop % maxHop;
		hop[idx].Set(time, bytes, qlen, rate, histogram);
		nhop++;
	}
}
...
void IntHeader::Serialize (Buffer::Iterator start) const{
	...
	if (mode == NORMAL){
		for (uint32_t j = 0; j < maxHop; j++){
			...
			for(uint8_t k = 0; k < 8; k++) i.WriteU8(hop[j].histogram[k]);
		}
		i.WriteU16(nhop);
	}else if (mode == TS){
		i.WriteU64(ts);
...
uint32_t IntHeader::Deserialize (Buffer::Iterator start){
	...
	if (mode == NORMAL){
		for (uint32_t j = 0; j < maxHop; j++){
			...
			for(uint8_t k = 0; k < 8; k++) hop[j].histogram[k] = i.ReadU8();
		}
		nhop = i.ReadU16();
	}else if (mode == TS){
		ts = i.ReadU64();
