...
public:
	...
	union{
		struct {
			uint64_t lineRate: 64-timeWidth-bytesWidth-qlenWidth,	//3bits
					 time: timeWidth,	//24bits
					 bytes: bytesWidth,	//20bits
					 qlen: qlenWidth;	//16bits
		};
		uint32_t buf[2];
	};
	//这里不放在联合体里面是否有区别？
	//由于使用uint32_t还是会多出64b的头部开销，先用64b实验
	uint8_t histogram[8];
	...
	void Set(uint64_t _time, uint64_t _bytes, uint32_t _qlen, uint64_t _rate, uint8_t _histogram[8]){
		time = _time;
		bytes = _bytes / (byteUnit * multi);
		qlen = _qlen / (qlenUnit * multi);
		for(uint8_t i = 0; i < 8; i++) histogram[i] = _histogram[i];
		switch (_rate){
			case 25000000000lu:
				lineRate=0;break;
			case 50000000000lu:
...
public:
	...
	void PushHop(uint64_t time, uint64_t bytes, uint32_t qlen, uint64_t rate, uint8_t histogram[8]);
