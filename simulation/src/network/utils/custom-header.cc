...
void CustomHeader::Serialize (Buffer::Iterator start) const{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  i.WriteU8(udp.cur_rate);
		  //i.WriteU8(udp.ignore);
		  udp.ih.Serialize(i);
...
uint32_t CustomHeader::GetAckSerializedSize(void){
	//return sizeof(ack.sport) + sizeof(ack.dport) + sizeof(ack.flags) + sizeof(ack.pg) + sizeof(ack.seq) + sizeof(ack.tag) + sizeof(ack.rate) + IntHeader::GetStaticSize();
...
uint32_t CustomHeader::GetUdpHeaderSize(void){
	...
	//return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.cur_rate) + sizeof(udp.ignore) + IntHeader::GetStaticSize();
	return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.cur_rate) + IntHeader::GetStaticSize();
	//return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.rate) + sizeof(udp.rateNew) + IntHeader::GetStaticSize();
	//return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.flowState) + sizeof(udp.flows) + IntHeader::GetStaticSize();
}
