...
public:
	...
	//
	//TracedCallback<Ptr<RdmaQueuePair>, uint32_t, uint32_t> m_traceSendPkt;
	TracedCallback<uint32_t, double, double, double, double, Ipv4Address, Ipv4Address> m_traceChangeRate;
	...
	//callback when send pkt by 
	//void SendPkt(Ptr<RdmaQueuePair> qp, uint32_t seq, uint32_t expectSeq);
	void changeRate(uint32_t seq, double new_rate, double old_rate, double cur_rate, double c, Ipv4Address sip, Ipv4Address dip);
};
